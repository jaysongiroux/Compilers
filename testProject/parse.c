...
/* checks for plus or minus token then creates a new node */
...
/* checks for multiplication and divison and creates a new node */
TreePtr term(void)
{ TreePtr t,p,q;
...
{ TreePtr t = NULL,p;
  char* savedName;
  switch (token)  /* finds parenthesis and saves expression */
  { case LPAREN:
    	match(LPAREN);
	t = expression();
	match(RPAREN);
    ...
    case ID:
      ...
      break;
    case NUM :   /* gets token and converts it into a integer*/
       t = newExpNode(ConstK);
